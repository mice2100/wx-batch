...
class WXBot:
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        if mtype == 1:
            if content.find('http://weixin.qq.com/cgi-bin/redirectforward?args=') != -1:
                ...
                if self.DEBUG:
                    logging.debug('    %s[Location] %s ' % (msg_prefix, pos))
            else:
                msg_content['type'] = 0
                ...
                if self.DEBUG:
                    try:
                        logging.debug('    %s[Text] %s' % (msg_prefix, msg_content['data']))
                    except UnicodeEncodeError:
                        logging.debug('    %s[Text] (illegal text).' % msg_prefix)
        elif mtype == 3:
            msg_content['type'] = 3
            ...
            if self.DEBUG:
                ...
                logging.debug('    %s[Image] %s' % (msg_prefix, image))
        elif mtype == 34:
            msg_content['type'] = 4
            ...
            if self.DEBUG:
                ...
                logging.debug('    %s[Voice] %s' % (msg_prefix, voice))
        elif mtype == 37:
            msg_content['type'] = 37
            msg_content['data'] = msg['RecommendInfo']
            if self.DEBUG:
                logging.debug('    %s[useradd] %s' % (msg_prefix,msg['RecommendInfo']['NickName']))
        elif mtype == 42:
            msg_content['type'] = 5
            ...
            if self.DEBUG:
                logging.debug('    %s[Recommend]' % msg_prefix)
                logging.debug('    -----------------------------')
                logging.debug('    | NickName: %s' % info['NickName'])
                logging.debug('    | Alias: %s' % info['Alias'])
                logging.debug('    | Local: %s %s' % (info['Province'], info['City']))
                logging.debug('    | Gender: %s' % ['unknown', 'male', 'female'][info['Sex']])
                logging.debug('    -----------------------------')
        elif mtype == 47:
            msg_content['type'] = 6
            msg_content['data'] = self.search_content('cdnurl', content)
            if self.DEBUG:
                logging.debug('    %s[Animation] %s' % (msg_prefix, msg_content['data']))
        elif mtype == 49:
            msg_content['type'] = 7
            if msg['AppMsgType'] == 3:
                app_msg_type = 'music'
            ...
            if self.DEBUG:
                logging.debug('    %s[Share] %s' % (msg_prefix, app_msg_type))
                logging.debug('    --------------------------')
                logging.debug('    | title: %s' % msg['FileName'])
                logging.debug('    | desc: %s' % self.search_content('des', content, 'xml'))
                logging.debug('    | link: %s' % msg['Url'])
                logging.debug('    | from: %s' % self.search_content('appname', content, 'xml'))
                logging.debug('    | content: %s' % (msg.get('content')[:20] if msg.get('content') else "unknown"))
                logging.debug('    --------------------------')
            ...
            msg_content['data'] = content
            if self.DEBUG:
                logging.debug('    %s[Video] Please check on mobiles' % msg_prefix)
        elif mtype == 53:
            msg_content['type'] = 9
            msg_content['data'] = content
            if self.DEBUG:
                logging.debug('    %s[Video Call]' % msg_prefix)
        elif mtype == 10002:
            msg_content['type'] = 10
            msg_content['data'] = content
            if self.DEBUG:
                logging.debug('    %s[Redraw]' % msg_prefix)
            ...
            if self.DEBUG:
                logging.debug('    %s[video] %s' % (msg_prefix, msg_content['data']))
        else:
            msg_content['type'] = 99
            msg_content['data'] = content
            if self.DEBUG:
                logging.debug('    %s[Unknown]' % msg_prefix)
        return msg_content
            ...
            try:
                ...
                if retcode == '1100':  # 从微信客户端上登出
                    ...
                    if selector == '2':  # 有新消息
                        ...
                        logging.debug('[DEBUG] sync_check %d %d:' % (retcode, selector))
                        r = self.sync()
                        if r is not None:
                            self.handle_msg(r)
                else:
    ...
    def add_friend_to_group(self,uid,group_name):
        ...
        logging.debug('[DEBUG] group_name:%s group_num:%s' % (group_name,group_num))
        #通过群id判断uid是否在群中
    ...
    def upload_media(self, fpath, is_img=False):
        if not os.path.exists(fpath):
            logging.error('[ERROR] File not exists.')
            return None
        ...
        try:
            ...
            if json.loads(r.text)['BaseResponse']['Ret'] != 0:
                logging.error('[ERROR] Upload media failure.')
                return None
    ...
    def send_file_msg_by_uid(self, fpath, uid):
        ...
        data = {
                ...
                'Msg': {
                    'Type': 6,
                    'Content': ("<appmsg appid='wxeb7ec651dd0aefa9' sdkver=''><title>%s</title><des></des><action></action><type>6</type><content></content><url></url><lowurl></lowurl><appattach><totallen>%s</totallen><attachid>%s</attachid><fileext>%s</fileext></appattach><extinfo></extinfo></appmsg>" % (os.path.basename(fpath), str(os.path.getsize(fpath)), mid, fpath.split('.')[-1])),
                    'FromUserName': self.my_account['UserName'],
                    'ToUserName': uid,
                    'LocalID': msg_id,
                    'ClientMsgId': msg_id, }, }
    ...
    def send_imgid_msg_by_uid(self, mid, uid, fpath):
        ...
        try:
            ...
            if res['BaseResponse']['Ret'] == 0:
                ...
                logging.debug("send image msg failed")
                return False
        except Exception as e:
            return False
    ...
    def send_img_msg_by_uid(self, fpath, uid):
        ...
        if mid is None:
            return False
        return self.send_imgid_msg_by_uid(mid, uid, fpath)
    ...
    def get_user_id(self, name):
        ...
        for contact in self.contact_list:
            ...
            elif 'RemarkName' in contact and contact['RemarkName'] == name:
                return contact['UserName']
    ...
    def send_msg(self, name, word, isfile=False):
        ...
        if uid is not None:
            ...
            if self.DEBUG:
                logging.debug('[ERROR] This user does not exist .')
            return True
    ...
    def run(self):
        try:
            ...
            if result != SUCCESS:
                logging.error('[ERROR] Web WeChat login failed. failed code=%s' % (result,))
            ...
            if self.login():
                ...
                logging.error('[ERROR] Web WeChat login failed .')
                self.status = 'loginout'
                return
            ...
            self.proc_msg()
            self.status = 'loginout'
            logging.info('[INFO] Quit process messages .')
        except Exception as e:
            logging.error('[ERROR] Web WeChat run failed --> %s'%(e))
            self.status = 'loginout'
    ...
    def wait4login(self):
        ...
        while retry_time > 0:
            ...
            if code == SCANED:
                ...
                logging.error(('[ERROR] WeChat login exception return_code=%s. retry in %s secs later...' %
                       (code, try_later_secs)))
                tip = 1
                retry_time -= 1
                time.sleep(try_later_secs)
